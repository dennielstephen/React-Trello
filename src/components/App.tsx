...
const App = () => {
  ...
  const onDragEnd = (result: any) => {
    ...
    dispatch(
      addLog({
        logId: v4(),
        logMessage: `Moved "${sourceList.tasks.filter(task => task.taskId === draggableId)[0]
          .taskName
          }" from list "${sourceList.listName}" to list "${lists.filter(list => list.listId === destination.droppableId)[0]
            .listName
          }"`,
        logAuthor: "Stip",
